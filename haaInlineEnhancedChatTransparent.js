...
export default class HaaInlineEnhancedChatTransparent extends LightningElement {
  @api orgId;
  ...
  // --- Transparency & color overrides ---
  @api inputBoxOpacity = "30";
  @api welcomeMessage = "";
  @api welcomeMessageColor = "";
  @api promptPlaceholder = "";
  @api promptPlaceholderColor = "";
  @api inputTextColor = "";
  ...
  get headingText() {
    return this.welcomeMessage || this.labels.HAA_heading;
  }

  get placeholderText() {
    return this.promptPlaceholder || this.labels.HAA_input_placeholder;
  }

  get headingStyle() {
    return this.welcomeMessageColor ? `color: ${this.welcomeMessageColor};` : "";
  }

  get inputRowStyle() {
    const pct = Math.min(100, Math.max(0, parseInt(this.inputBoxOpacity, 10) || 30));
    const alpha = (pct / 100).toFixed(2);
    const bg = `rgba(255, 255, 255, ${alpha})`;
    const placeholder = this.promptPlaceholderColor
      ? `--haa-placeholder-color: ${this.promptPlaceholderColor};`
      : "";
    return `background: ${bg}; ${placeholder}`;
  }

  get inputTextStyle() {
    return this.inputTextColor ? `color: ${this.inputTextColor};` : "";
  }
